...
{
    public class EF_NumberingGRN_Repository : I_Numbering_Repository
    {
        public IEnumerable<object> NumberingList
        {
            get
            {
                ...
                {
                    return dbcontext.NumberingGRN.AsNoTracking().ToList();
                }
            }
        }
        public void SaveNumberings(object ParamNumberingObj)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NumberingGRN NumberingObj = (NumberingGRN)ParamNumberingObj;
                    ...
                    using (DbContextTransaction transaction = dbcontext.Database.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
                    {
                        if (NumberingObj.NumberingID == 0)
                        {
                            if (NumberingObj.IsDefault.Equals(true))
                            {
                                ...
                                dbcontext.Database.ExecuteSqlCommand(@"update NumberingGRN set IsDefault=@IsDefault", new SqlParameter("@IsDefault", false));
                            }
                            int DocEntry = dbcontext.NumberingGRN.Count() + 1;
                            ...
                            dbcontext.NumberingGRN.Add(NumberingObj);
                        ...
                        else
                        {
                            ...
                            NumberingGRN dbEntry = dbcontext.NumberingGRN.Find(NumberingObj.NumberingID);
                            if (dbEntry != null)
                            {
        ...
        {
            NumberingGRN NoteObject = null;
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingGRN.AsNoTracking().Where(i => !i.NumberingID.Equals(NumberingID) && i.SeriesName.Equals(SeriesName)).FirstOrDefault();
            }
        ...
        public bool IsSeriesOverlaps(int FirstNo, int LastNo, int NumberingID = 0)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingGRN.AsNoTracking().Where(i => ((FirstNo >= i.FirstNo && FirstNo <= i.LastNo) || (LastNo >= i.FirstNo && LastNo <= i.LastNo)) && !i.NumberingID.Equals(NumberingID)).FirstOrDefault();
            }
            return NoteObject == null ? false : true;
        }
        public bool IsDefault(int NumberingID)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingGRN.AsNoTracking().Where(i => i.NumberingID.Equals(NumberingID)).FirstOrDefault();
            }
        ...
        public void Remove(int numberingID)
        {
            using (var dbcontext = new DomainDb())
            {
                NumberingGRN dbEntry = dbcontext.NumberingGRN.Find(numberingID);
                if (dbEntry != null)
                {
                    dbcontext.NumberingGRN.Remove(dbEntry);
                    dbcontext.SaveChanges();
                }
            }
        }
