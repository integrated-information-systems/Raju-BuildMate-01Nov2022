...
{
    public class EF_NumberingSQ_Repository : I_Numbering_Repository
    {
        public IEnumerable<object> NumberingList
        {
            get
            {
                ...
                {
                    return dbcontext.NumberingSQ.AsNoTracking().ToList();
                }
            }
        }
        public void SaveNumberings(object ParamNumberingObj)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NumberingSQ NumberingObj = (NumberingSQ)ParamNumberingObj;
                    ...
                    using (DbContextTransaction transaction = dbcontext.Database.BeginTransaction(System.Data.IsolationLevel.ReadCommitted))
                    {
                        if (NumberingObj.NumberingID == 0)
                        {
                            if(NumberingObj.IsDefault.Equals(true))
                            {
                                ...
                                dbcontext.Database.ExecuteSqlCommand(@"update NumberingSQ set IsDefault=@IsDefault", new SqlParameter("@IsDefault", false));
                            }
                            int DocEntry = dbcontext.NumberingSQ.Count() + 1;
                            ...
                            dbcontext.NumberingSQ.Add(NumberingObj);
                        ...
                        else
                        {
                            ...
                            NumberingSQ dbEntry = dbcontext.NumberingSQ.Find(NumberingObj.NumberingID);
                            if (dbEntry != null)
                            {
        ...
        {
            NumberingSQ NoteObject = null;
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingSQ.AsNoTracking().Where(i => !i.NumberingID.Equals(NumberingID) && i.SeriesName.Equals(SeriesName)).FirstOrDefault();
            }
        ...
        public bool IsSeriesOverlaps(int FirstNo, int LastNo, int NumberingID = 0)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingSQ.AsNoTracking().Where(i => ((FirstNo >= i.FirstNo && FirstNo <= i.LastNo) || (LastNo >= i.FirstNo && LastNo <= i.LastNo)) && !i.NumberingID.Equals(NumberingID)).FirstOrDefault();
            }
            return NoteObject == null ? false : true;
        }
        public bool IsDefault(int NumberingID)
        {
            ...
            using (var dbcontext = new DomainDb())
            {
                NoteObject = dbcontext.NumberingSQ.AsNoTracking().Where(i => i.NumberingID.Equals(NumberingID)).FirstOrDefault();
            }
        ...
        public void Remove(int numberingID)
        {
            using (var dbcontext = new DomainDb())
            {
                NumberingSQ dbEntry = dbcontext.NumberingSQ.Find(numberingID);
                if (dbEntry != null)
                {
                    dbcontext.NumberingSQ.Remove(dbEntry);
                    dbcontext.SaveChanges();
                }
            }
        }
