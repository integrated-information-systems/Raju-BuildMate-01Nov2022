...
{
    [Authorize]
    public class CNoteMngtController : Controller
    {
        ...
        IMapper _mapper;
        public CNoteMngtController([CustomerNoteMngt] I_Notes_Repository i_Notes_Repository, IMapper _mapper)
        {
        ...
        {
            IEnumerable<object> cNotesMngt = i_Notes_Repository.GetNotesListByCardCode(CardCode);
            IEnumerable<NoteViewModel> CNotesMngtList = _mapper.Map<IEnumerable<object>, IEnumerable<NoteViewModel>>(cNotesMngt);
            NoteListViewModel ListModel = new NoteListViewModel() { NotesList = CNotesMngtList, AjaxOptions = ajaxFormViewModel, CanEditNote = User.IsInRole("Notes") ? true : false };
        ...
        public ActionResult AddNote(AddUpdateNoteViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NoteID == 0)
                {
                    ...
                    CNotesMngt NoteObject = _mapper.Map<AddUpdateNoteViewModel, CNotesMngt>(model);
                ...
                else
                {
                    ...
                    CNotesMngt NoteObject = (CNotesMngt)i_Notes_Repository.GetNote(model.NoteID);
                    if (NoteObject != null)
                    {
            ...
            {
                IEnumerable<Object> CNotesMngt = i_Notes_Repository.GetNotesListByCardCode(model.CardCode);
                IEnumerable<NoteViewModel> CNotesMngtList = _mapper.Map<IEnumerable<Object>, IEnumerable<NoteViewModel>>(CNotesMngt);

                NoteListViewModel ListModel = new NoteListViewModel() { NotesList = CNotesMngtList, AjaxOptions = ajaxFormViewModel };
        ...
        public ActionResult EditNote(int NoteID)
        {
            ...
            if (ModelState.IsValid)
            {
                CNotesMngt NoteObject = (CNotesMngt)i_Notes_Repository.GetNote(NoteID);
                if (NoteObject != null)
                {
                    EditNoteModel = _mapper.Map<CNotesMngt, AddUpdateNoteViewModel>(NoteObject);
