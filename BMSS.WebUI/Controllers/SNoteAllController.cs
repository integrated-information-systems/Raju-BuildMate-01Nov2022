...
{
    [Authorize]
    public class SNoteAllController : Controller
    {
        ...
        IMapper _mapper;
        public SNoteAllController([SupplierNoteAll] I_Notes_Repository i_Notes_Repository, IMapper _mapper)
        {
        ...
        {
            IEnumerable<object> sNotesAll = i_Notes_Repository.GetNotesListByCardCode(CardCode);
            IEnumerable<NoteViewModel> SNotesAllList = _mapper.Map<IEnumerable<object>, IEnumerable<NoteViewModel>>(sNotesAll);
            NoteListViewModel ListModel = new NoteListViewModel() { NotesList = SNotesAllList, AjaxOptions = ajaxFormViewModel, CanEditNote = User.IsInRole("Notes") ? true : false };
        ...
        public ActionResult AddNote(AddUpdateNoteViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NoteID == 0)
                {
                    ...
                    SNotesAll NoteObject = _mapper.Map<AddUpdateNoteViewModel, SNotesAll>(model);
                ...
                else
                {
                    ...
                    SNotesAll NoteObject = (SNotesAll)i_Notes_Repository.GetNote(model.NoteID);
                    if (NoteObject != null)
                    {
            ...
            if (jsonResultViewModel.IsOpertationSuccess)
            {
                ...
                IEnumerable<NoteViewModel> SNotesAllList = _mapper.Map<IEnumerable<Object>, IEnumerable<NoteViewModel>>(cNotesAll);

                NoteListViewModel ListModel = new NoteListViewModel() { NotesList = SNotesAllList, AjaxOptions = ajaxFormViewModel };
        ...
        public ActionResult EditNote(int NoteID)
        {
            ...
            if (ModelState.IsValid)
            {
                SNotesAll NoteObject = (SNotesAll)i_Notes_Repository.GetNote(NoteID);
                if (NoteObject != null)
                {
                    EditNoteModel = _mapper.Map<SNotesAll, AddUpdateNoteViewModel>(NoteObject);
