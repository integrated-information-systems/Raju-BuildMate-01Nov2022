...
{
    public class NumberingSTController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingST",
            AddActionTargetId = "NumberingSTListTble",
            AddFormId = "NumberingSTAddForm",
            FormDeleteAction = "DeleteNumberingST",
            DeleteActionTargetId = "NumberingSTListTble",
            DeleteFormId = "NumberingSTDeleteForm",
            FormEditAction = "EditNumberingST",
            EditActionTargetId = "NumberingSTListTble",
            EditFormId = "NumberingSTEditForm",
            FormUpdateAction = "AddNumberingST",
            UpdateActionTargetId = "NumberingSTListTble",
            UpdateFormId = "NumberingSTAddForm",
            ModalID = "AddUpdateNumberingSTModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingSTController([NumST] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingST(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingST NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingST>(model);
                ...
                else
                {
                    ...
                    NumberingST NumberingObject = (NumberingST)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                            ...
                            else
                            {
                                ...
                                NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingST>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingST(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingST NumberingObject = (NumberingST)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {
                        EditNumberingModel = _mapper.Map<NumberingST, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingST(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
