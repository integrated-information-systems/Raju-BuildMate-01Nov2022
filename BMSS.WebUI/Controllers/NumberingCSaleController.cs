...
{
    public class NumberingCSaleController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingCSale",
            AddActionTargetId = "NumberingCSaleListTble",
            AddFormId = "NumberingCSaleAddForm",
            FormDeleteAction = "DeleteNumberingCSale",
            DeleteActionTargetId = "NumberingCSaleListTble",
            DeleteFormId = "NumberingCSaleDeleteForm",
            FormEditAction = "EditNumberingCSale",
            EditActionTargetId = "NumberingCSaleListTble",
            EditFormId = "NumberingCSaleEditForm",
            FormUpdateAction = "AddNumberingCSale",
            UpdateActionTargetId = "NumberingCSaleListTble",
            UpdateFormId = "NumberingCSaleAddForm",
            ModalID = "AddUpdateNumberingCSaleModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingCSaleController([NumCSale] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingCSale(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingCSale NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingCSale>(model);
                ...
                else
                {
                    ...
                    NumberingCSale NumberingObject = (NumberingCSale)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                            ...
                            else
                            {
                                ...
                                NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingCSale>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingCSale(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingCSale NumberingObject = (NumberingCSale)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {
                        EditNumberingModel = _mapper.Map<NumberingCSale, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingCSale(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
