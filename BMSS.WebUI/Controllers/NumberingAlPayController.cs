...
{
    public class NumberingAlPayController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingAlPay",
            AddActionTargetId = "NumberingAlPayListTble",
            AddFormId = "NumberingAlPayAddForm",
            FormDeleteAction = "DeleteNumberingAlPay",
            DeleteActionTargetId = "NumberingAlPayListTble",
            DeleteFormId = "NumberingAlPayDeleteForm",
            FormEditAction = "EditNumberingAlPay",
            EditActionTargetId = "NumberingAlPayListTble",
            EditFormId = "NumberingAlPayEditForm",
            FormUpdateAction = "AddNumberingAlPay",
            UpdateActionTargetId = "NumberingAlPayListTble",
            UpdateFormId = "NumberingAlPayAddForm",
            ModalID = "AddUpdateNumberingAlPayModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingAlPayController([NumAlPay] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingAlPay(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingAlPay NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingAlPay>(model);
                ...
                else
                {
                    ...
                    NumberingAlPay NumberingObject = (NumberingAlPay)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                                ...
                                else
                                {
                                    ...
                                    NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingAlPay>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingAlPay(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingAlPay NumberingObject = (NumberingAlPay)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {
                        EditNumberingModel = _mapper.Map<NumberingAlPay, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingAlPay(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
