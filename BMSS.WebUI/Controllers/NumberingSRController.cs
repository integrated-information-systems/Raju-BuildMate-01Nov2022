...
{
    public class NumberingSRController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingSR",
            AddActionTargetId = "NumberingSRListTble",
            AddFormId = "NumberingSRAddForm",
            FormDeleteAction = "DeleteNumberingSR",
            DeleteActionTargetId = "NumberingSRListTble",
            DeleteFormId = "NumberingSRDeleteForm",
            FormEditAction = "EditNumberingSR",
            EditActionTargetId = "NumberingSRListTble",
            EditFormId = "NumberingSREditForm",
            FormUpdateAction = "AddNumberingSR",
            UpdateActionTargetId = "NumberingSRListTble",
            UpdateFormId = "NumberingSRAddForm",
            ModalID = "AddUpdateNumberingSRModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingSRController([NumSR] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingSR(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingSR NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingSR>(model);
                ...
                else
                {
                    ...
                    NumberingSR NumberingObject = (NumberingSR)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                            ...
                            else
                            {
                                ...
                                NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingSR>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingSR(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingSR NumberingObject = (NumberingSR)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {

                        EditNumberingModel = _mapper.Map<NumberingSR, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingSR(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
