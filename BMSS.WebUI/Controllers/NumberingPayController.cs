...
{
    public class NumberingPayController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingPay",
            AddActionTargetId = "NumberingPayListTble",
            AddFormId = "NumberingPayAddForm",
            FormDeleteAction = "DeleteNumberingPay",
            DeleteActionTargetId = "NumberingPayListTble",
            DeleteFormId = "NumberingPayDeleteForm",
            FormEditAction = "EditNumberingPay",
            EditActionTargetId = "NumberingPayListTble",
            EditFormId = "NumberingPayEditForm",
            FormUpdateAction = "AddNumberingPay",
            UpdateActionTargetId = "NumberingPayListTble",
            UpdateFormId = "NumberingPayAddForm",
            ModalID = "AddUpdateNumberingPayModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingPayController([NumPay] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingPay(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingPay NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingPay>(model);
                ...
                else
                {
                    ...
                    NumberingPay NumberingObject = (NumberingPay)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                            ...
                            else
                            {
                                ...
                                NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingPay>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingPay(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingPay NumberingObject = (NumberingPay)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {
                        EditNumberingModel = _mapper.Map<NumberingPay, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingPay(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
