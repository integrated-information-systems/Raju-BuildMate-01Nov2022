...
{
    public class NumberingDOController : Controller
    {
        private AjaxFormViewModel ajaxFormViewModel = new AjaxFormViewModel()
        {
            FormAddAction = "AddNumberingDO",
            AddActionTargetId = "NumberingDOListTble",
            AddFormId = "NumberingDOAddForm",
            FormDeleteAction = "DeleteNumberingDO",
            DeleteActionTargetId = "NumberingDOListTble",
            DeleteFormId = "NumberingDODeleteForm",
            FormEditAction = "EditNumberingDO",
            EditActionTargetId = "NumberingDOListTble",
            EditFormId = "NumberingDOEditForm",
            FormUpdateAction = "AddNumberingDO",
            UpdateActionTargetId = "NumberingDOListTble",
            UpdateFormId = "NumberingDOAddForm",
            ModalID = "AddUpdateNumberingDOModal"
        };
        //Repositories
        private I_Numbering_Repository i_Numbering_Repository;
        IMapper _mapper;

        public NumberingDOController([NumDO] I_Numbering_Repository i_Numbering_Repository, IMapper _mapper)
        {
        ...
        public ActionResult AddNumberingDO(AddUpdateDocNumberingViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (model.NumberingID == 0)
                {
                    ...
                    {
                        NumberingDO NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingDO>(model);
                ...
                else
                {
                    ...
                    NumberingDO NumberingObject = (NumberingDO)i_Numbering_Repository.GetNumbering(model.NumberingID);
                    if (NumberingObject != null)
                    {
                        if (NumberingObject.IsLocked.Equals(false))
                        {
                            ...
                            else
                            {
                                ...
                                NumberingObject = _mapper.Map<AddUpdateDocNumberingViewModel, NumberingDO>(model);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult EditNumberingDO(int NumberingID)
        {
            ...
            if (ModelState.IsValid)
            {
                NumberingDO NumberingObject = (NumberingDO)i_Numbering_Repository.GetNumbering(NumberingID);
                if (NumberingObject != null)
                {
                    if (NumberingObject.IsLocked.Equals(false))
                    {

                        EditNumberingModel = _mapper.Map<NumberingDO, AddUpdateDocNumberingViewModel>(NumberingObject);
                    }
                    else
                    {
                        ErrList.Add("Series already locked, cannot proceed");
        ...
        [HttpPost]
        [ValidateAntiForgeryToken()]
        public ActionResult DeleteNumberingDO(int NumberingID)
        {
            JsonResultViewModel jsonResultViewModel = new JsonResultViewModel() { };
