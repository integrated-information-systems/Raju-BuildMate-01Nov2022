...
{
    public partial class DOViwer : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (User.Identity.IsAuthenticated)
            {
                ...
                if (DOHeader != null)
                {
                    if (DOHeader.DOPrintedCount >= 1 && !User.IsInRole("Print After First Time"))
                    {
                        Response.Redirect("~");
                    }
                    else
                    {
                        string DocNum = i_DODocH_Repository.UpdatePrintStatus(DocEntry, User.Identity.Name);

                        string pdfFileName = "BM_DO" + "_" + DocNum + "_" + DateTime.Now.ToString("ddMMyy") + ".pdf";
                        URL = baseUrl + "WForms/ViewerPrint.aspx?DE=" + DocNum + "&Ty=" + BMSS.WebUI.Models.General.PrintDocTypes.Delivery_Order;

                        ReportDocument cryRpt = new ReportDocument();

                        cryRpt.Load(Server.MapPath("~\\App_Data\\Delivery_Order.rpt"));
                        ...
                        {                            
                            expCryRpt.Load(Server.MapPath("~\\App_Data\\Delivery_Order.rpt"));
